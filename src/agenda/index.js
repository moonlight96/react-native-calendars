...
import {Text, View, Dimensions, Animated, ViewPropTypes,TouchableOpacity,findNodeHandle,UIManager} from 'react-native';
...
export default class AgendaView extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      // xViewHeight:'',
      dotTop:this.viewWidth/2-(this.getMonthWeek(moment().year(),moment().month()-1,moment().date())-1)*42+3,
    };
    ...
    // this.state.scrollX.addListener(({value}) => this.knobTracker.add(value));
  }
  ...
  initialScrollPadPosition() {
    return Math.max(0, this.viewHeight - HEADER_HEIGHT);//关闭状态的calendarList高度
  }
  ...
  onLayout(event) {
    // this.setState({
    //   xViewHeight:event.nativeEvent.layout.height
    // })
    // console.warn(222,event.nativeEvent.layout.height);
...
getWeek(a, b, c) {
  ...
  //a + b 的和在除以7 就是当天是当前月份的第几周
  var date = new Date(a, parseInt(b) - 1, c), w = date.getDay(), d = date.getDate();
  return Math.floor((d + 6 - w) / 7);
};
  chooseDay(d, optimisticScroll,e) {
    const day = parseDate(d);
    const week=this.getMonthWeek(d.year,d.month,d.day)-1;
    this.setState({
      calendarScrollable: false,
      selectedDay: day.clone(),
      dotTop:this.viewWidth/2-week*42+3
    });
    // this.forceUpdate()
  ...
  onDayChange(day) {
    ...
    const newXDate=xdateToData(newDate)
    const withAnimation = dateutils.sameMonth(newDate, this.state.selectedDay);
    const week=this.getWeek(newXDate.year,newXDate.month-1,newXDate.day);
    // console.warn(week,newXDate)
    ...
    return (
      <View testID={this.props.testID} onLayout={this.onLayout} style={[this.props.style, {flex: 1, overflow: 'hidden'}]}>
        ...
        <Animated.View style={headerStyle}>
          ...
          {/* paddingTop:this.state.calendarScrollable?50:this.state.dotTop,  */}
            <CalendarList
              onLayout={() => {
